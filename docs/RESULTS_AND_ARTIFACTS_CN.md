...
用于仓库内部的 STGCN、DCRNN、STaR-GNN 对比、消融、逐日分析和运行解释。先在每个样本-小时将 DMA A--J 的真实值和预测值分别求和，再对总需求序列统一计算 MAE、MAPE、RMSE 和 NSE。
...
对应文件：`paper/tables/literature/table_literature_comparison_common46.*`。为兼容旧路径，`table_comparison_common46.*` 保留为同一张文献对比表的别名。
...
## 5. 与 MSCMNet 原文同口径的文献对比

`configs/evaluation/mscmnet_literature_totals.yaml` 保存从 Que et al. (2024) Supplementary Tables S1-1--S1-8 转录的 GRU、LSTM、MSNet、MSCMNet_WM、MSCMNet_M、MSCMNet_W `total` 结果及来源说明。

当前 publisher-compatible 表中，STaR-GNN 为：
...
该表用于跨论文结果对照；不得用 aggregate-demand MAE `4.360841/4.919812` 与 MSCMNet 补充材料中的 `14.471/14.950` 直接计算相对提升。
...
| `paper/tables/literature/table_literature_comparison_common46.*` | 与 GRU/LSTM/MSNet/MSCMNet 的 publisher-compatible 文献对比 |
...
总体图、消融图和 Day 1--Day 7 图均使用统一 aggregate-demand 口径；DMA 图使用各 DMA 自身指标，因此不需要改成 publisher-compatible MAE。
...
```text
Paper tables: .../paper/tables/literature
