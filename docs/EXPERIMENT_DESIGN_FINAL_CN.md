...
- Fig. S1：按完整数据质量统计选择的清洗前后代表性曲线，用于记录缺失值插值和训练期 IQR 阈值处理；
- Fig. S2：低、中、高平均需水 DMA 的典型周模式，用于连接数据周期结构与 SAS-Norm 的设计动机；
- Fig. S3：DCRNN、STGCN、STaR-GNN 在 46 个共同预测起点上的 MAE 经验累积分布，用于补充 Main Fig. 6 的总体分布证据。

三张补充图分别承担数据质量、需求规律和误差分布，不增加正文主图数量，也不替代主文的绝对指标、配对效应或置信区间分析。
