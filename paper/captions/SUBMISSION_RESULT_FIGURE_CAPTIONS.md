...
**Overall forecasting performance across prediction horizons and model families.** **a**, Relative reductions in MAE, MAPE and RMSE achieved by STaR-GNN against GRU, LSTM, MSNet, three MSCMNet variants, DCRNN and STGCN for the 24 h and 168 h tasks. **b**, Corresponding absolute improvements in NSE, defined as \(\Delta\mathrm{NSE}=\mathrm{NSE}_{\mathrm{STaR-GNN}}-\mathrm{NSE}_{\mathrm{baseline}}\). Error-metric reductions are calculated as \((E_{baseline}-E_{STaR-GNN})/E_{baseline}\times100\%\); positive values consistently favor STaR-GNN. Values for GRU, LSTM, MSNet and the MSCMNet variants are taken from Que et al. (2024), whereas DCRNN, STGCN and STaR-GNN are evaluated using the common 46-origin protocol.
...
**DMA-level improvement of STaR-GNN across four metrics.** Relative MAE, MAPE and RMSE reductions and absolute NSE changes \(\Delta\mathrm{NSE}\) are shown against DCRNN and STGCN for each DMA at 24 h and 168 h. Blue denotes improvement and red denotes deterioration. The detailed absolute values are reported in Supplementary Table S1.
