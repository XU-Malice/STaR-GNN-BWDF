...
Fig. 3 单独呈现 24 h，Fig. 4 单独呈现 168 h；每张图均采用 2 × 2 分面，对应 MAE、MAPE、RMSE 和 NSE。每个 DMA 使用蓝色方点和灰色空心圆直接比较 STaR-GNN 与该“预测时域–DMA–指标”组合中最强非 STaR-GNN 方法的绝对值，竖向线段表示二者差距；局部最优基线占优时，空心圆及连线改为橙色。相同指标在两个预测时域中使用一致的聚焦纵轴，既支持直接比较，也避免宽泛坐标范围压缩局部差异。
