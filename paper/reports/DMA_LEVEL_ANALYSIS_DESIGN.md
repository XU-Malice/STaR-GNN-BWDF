...
1. STaR-GNN 相对各类对比模型的改善是否广泛分布于不同 DMA，而非由少数高需水分区主导？
...
| 1:1 paired scatter | 提出方法是否优于一个指定对比模型？ | JoH/WRR 常见，例外清楚 | 八个对比模型和四个指标会形成大量 panel；误差与 NSE 的优劣方向不同 | 不作为主图 |
| signed pairwise distribution | 相对每个对比模型的改善是否跨 DMA 广泛存在？ | 保留差距、方向和模型族；不混合 DMA 绝对尺度 | 不直接标出异常 DMA | 采用为 Fig. 2 |
| absolute best-comparator bars | 每个 DMA 上 STaR-GNN 与最优局部替代方案的绝对性能如何？ | 直接保留指标量纲、DMA 身份和 24/168 h 差异 | 不直接编码相对改善幅度 | 按预测时域拆分为 Figs. 3–4 |
...
四个 panel 分别为 MAE、MAPE、RMSE 和 NSE。纵轴为八种对比模型，横轴为 STaR-GNN 相对相应对比模型的有符号改善；误差指标使用相对降幅，NSE 使用绝对提升。每个小点代表一个 DMA，大点为中位数，线段为四分位距；圆和方形分别表示 24 h 与 168 h。六种时序模型与两种图模型之间用水平线分隔。

该图回答“改善是否跨模型族和 DMA 广泛存在”，并替代原排名分布。零线左侧的数据完整保留。

### Main Figs. 3–4 — horizon-specific absolute DMA performance

Fig. 3 单独呈现 24 h，Fig. 4 单独呈现 168 h；每张图均采用 2 × 2 分面，对应 MAE、MAPE、RMSE 和 NSE。每个 DMA 使用一组并列柱直接比较 STaR-GNN 与该“预测时域–DMA–指标”组合中最强非 STaR-GNN 方法的绝对值；STaR-GNN 使用蓝色，对比模型使用灰色，后者占优时改为橙色，并在柱顶简要标注模型名称。全部纵轴从零开始，并通过更细的主、次网格辅助读取相近数值。

两张图在 Fig. 2 的分布证据之上增加三个信息：具体 DMA 身份、绝对误差或拟合水平，以及最优替代模型的局部例外。因此三图形成“跨 DMA 改善分布 → 日前绝对性能 → 周尺度局部竞争边界”的递进关系，而不是重复统计胜场。

按 Elsevier 图件规范，两张主图不在图内重复标题、计算式或长句解释；图内仅保留面板字母、指标、坐标轴和简短图例，符号、正负方向与竞争者选择规则均由自包含图注说明。

## 5. 数据完整性和解释边界

输入表包含 180 行，即 2 个预测时域 × 10 个 DMA × 9 个模型；主键无重复、四项指标无缺失，每个预测时域–DMA 组合均含九种模型。Figure 2 在每个预测时域内使用 \(10\text{ DMAs}\times8\text{ comparison models}\times4\text{ metrics}=320\) 个有符号描述性对比，两个预测时域合计 640 个。这里的“320”是覆盖度的计数单位，不是 320 个相互独立的统计样本，也不用于显著性推断。Figures 3–4 各使用 \(10\times4=40\) 个逐单元最强竞争者比较，两个预测时域合计 80 个。
...
| 图离开正文后无法理解正负方向和符号 | 坐标轴保留指标与单位；自包含 caption 定义点、区间、连线、橙色例外、误差降幅和 \(\Delta\mathrm{NSE}\) | 通过 |
| Fig. 2 与 Figs. 3–4 重复“多数获胜” | Fig. 2 回答相对各对比模型的跨 DMA 覆盖；Figs. 3–4 分时域呈现与局部最优对比模型的绝对指标和例外位置 | 通过 |
...
完整绝对数值保留在 Supplementary Table S1；局部最强竞争者身份与精确幅度保留在 Supplementary Table S2。旧排名图和首位计数热图因信息损失和相互重复，不再作为正文或补充图件。
