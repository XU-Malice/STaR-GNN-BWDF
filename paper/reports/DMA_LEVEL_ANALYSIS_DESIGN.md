...
| absolute best-baseline bars | 每个 DMA 上 STaR-GNN 与最优局部替代方案的绝对性能如何？ | 直接保留指标量纲、DMA 身份和 24/168 h 差异 | 不直接编码相对改善幅度 | 采用为 Fig. 3 |
...
图按四行两列组织：四行依次对应 MAE、MAPE、RMSE 和 NSE，两列分别对应 24 h 与 168 h。每个 DMA 使用成对柱直接比较 STaR-GNN 与该“预测时域–DMA–指标”组合中最强非 STaR-GNN 方法的绝对值；灰色表示最优基线，橙色表示该基线优于 STaR-GNN，并就地标注模型名称。

该图在 Fig. 2 的分布证据之上增加三个信息：具体 DMA 身份、绝对误差或拟合水平，以及最优替代模型的局部例外。因此两图形成“跨 DMA 改善分布 → DMA 绝对性能与局部竞争边界”的递进关系，而不是重复统计胜场。
...
| Fig. 2 与 Fig. 3 重复“多数获胜” | Fig. 2 回答相对各基线的跨 DMA 覆盖；Fig. 3 直接呈现与局部最优基线的绝对指标和例外位置 | 通过 |
