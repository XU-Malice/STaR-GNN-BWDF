...
Fig. 3 单独呈现 24 h，Fig. 4 单独呈现 168 h；每张图均采用 2 × 2 分面，对应 MAE、MAPE、RMSE 和 NSE。每个 DMA 使用一组并列柱直接比较 STaR-GNN 与该“预测时域–DMA–指标”组合中最强非 STaR-GNN 方法的绝对值；STaR-GNN 使用蓝色，对比模型使用灰色，后者占优时改为橙色。全部纵轴从零开始，并通过更细的主、次网格辅助读取相近数值。
