...
| absolute best-baseline bars | 每个 DMA 上 STaR-GNN 与最优局部替代方案的绝对性能如何？ | 直接保留指标量纲、DMA 身份和 24/168 h 差异 | 不直接编码相对改善幅度 | 按预测时域拆分为 Figs. 3–4 |
...
Fig. 3 单独呈现 24 h，Fig. 4 单独呈现 168 h；每张图均采用 2 × 2 分面，对应 MAE、MAPE、RMSE 和 NSE。每个 DMA 使用成对柱直接比较 STaR-GNN 与该“预测时域–DMA–指标”组合中最强非 STaR-GNN 方法的绝对值；灰色表示最优基线，橙色表示该基线优于 STaR-GNN。两个预测时域不共用纵轴，以避免单图拥挤和尺度压缩。
...
输入表包含 180 行，即 2 个预测时域 × 10 个 DMA × 9 个模型；主键无重复、四项指标无缺失，每个预测时域–DMA 组合均含九种模型。Figure 2 在每个预测时域内使用 \(10\text{ DMAs}\times8\text{ baselines}\times4\text{ metrics}=320\) 个有符号描述性对比，两个预测时域合计 640 个。这里的“320”是覆盖度的计数单位，不是 320 个相互独立的统计样本，也不用于显著性推断。Figures 3–4 各使用 \(10\times4=40\) 个逐单元最强竞争者比较，两个预测时域合计 80 个。
...
| Fig. 2 与 Figs. 3–4 重复“多数获胜” | Fig. 2 回答相对各基线的跨 DMA 覆盖；Figs. 3–4 分时域呈现与局部最优基线的绝对指标和例外位置 | 通过 |
